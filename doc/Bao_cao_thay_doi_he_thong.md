...
* **Đối tượng tham gia**: Cả người dùng có vai trò `SPECTATOR`, `HORSE_OWNER`, và `JOCKEY` đều được phép đặt cược. Tuy nhiên, `HORSE_OWNER` và `JOCKEY` chỉ được phép đặt cược ở các giải đấu mà họ **không tham gia thi đấu**. Các vai trò khác (`RACE_REFEREE`, `ADMIN`) hoàn toàn bị cấm đặt cược.
...
### 1. API Đặt cược (Place Bet)
* **Endpoint**: `POST /api/bets`
* **Quyền truy cập (Authorization)**: Dành cho các vai trò `SPECTATOR`, `HORSE_OWNER`, và `JOCKEY`.
* **Request Body**:
  ```json
  {
    "raceId": 12,
...
* **Các ràng buộc mới bổ sung (New Validations)**:
  1. **Ràng buộc vai trò (Role-based Restriction)**: Hệ thống kiểm tra vai trò của User. Chỉ cho phép các vai trò `SPECTATOR`, `HORSE_OWNER`, và `JOCKEY` được đặt cược. Nếu là vai trò khác (như `ADMIN`, `RACE_REFEREE`), hệ thống trả về lỗi `403 Forbidden` với thông điệp: `"Chỉ người xem (SPECTATOR), chủ ngựa (HORSE_OWNER) và nài ngựa (JOCKEY) mới được phép đặt cược."`
  2. **Ràng buộc tham gia giải đấu (Tournament Participation Restriction)**:
     - Đối với `JOCKEY`: Nếu nài ngựa đang tham gia bất kỳ cuộc đua nào trong cùng giải đấu của cuộc đua được chọn, hệ thống trả về lỗi `403 Forbidden` với thông điệp: `"Nài ngựa (JOCKEY) không được phép đặt cược trong giải đấu mà mình tham gia."`
     - Đối với `HORSE_OWNER`: Nếu chủ ngựa sở hữu bất kỳ chú ngựa nào tham gia giải đấu đó, hệ thống trả về lỗi `403 Forbidden` với thông điệp: `"Chủ ngựa (HORSE_OWNER) không được phép đặt cược trong giải đấu mà mình tham gia."`
...
| :--- | :--- | :--- |
| [BetService.java](file:///Users/minhvu2201/Documents/horse-racing-system/backend/src/main/java/com/horseracing/services/BetService.java) | Logic nghiệp vụ | • Cho phép `SPECTATOR`, `HORSE_OWNER`, và `JOCKEY` đặt cược.<br>• Chặn `HORSE_OWNER` và `JOCKEY` cược trong giải đấu mà họ tham gia thi đấu.<br>• Thay đổi trạng thái cuộc đua hợp lệ để đặt cược thành `LOCKED_LIST`.<br>• Chặn đặt cược nếu quá trình mô phỏng cuộc đua đã kết thúc. |
